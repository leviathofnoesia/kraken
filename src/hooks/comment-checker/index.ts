...
function checkCommentsInCode(
  ...
  const delimiters = language
    ? LANGUAGE_COMMENT_DELIMITERS[language as keyof typeof LANGUAGE_COMMENT_DELIMITERS]
    : null

  let inMultiLine = false
  let _multiLineStartLine = 0
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      delimiters &&
      'shebang' in delimiters &&
      Array.isArray(delimiters.shebang) &&
      delimiters.shebang.length > 0
    ) {
...
export function createCommentChecker(
  ...
  return {
    ...
    'tool.execute.after': async (input, output) => {
      if (!config.enabled || !config.checkAfterToolUse) {
        return
      }
