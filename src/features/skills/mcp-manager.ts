...
export class SkillMcpManager {
  ...
  async getOrCreateClient(
    ...
    // @ts-ignore - Client constructor options changed in new SDK
  ...
  async callTool(
    ...
    try {
      const result = await client.callTool(name, args) ?? null
  ...
  async readResource(
    ...
    try {
      const result = await client.readResource(uri) ?? null
  ...
  async getPrompt(
    ...
    try {
      const result = await client.getPrompt(name, args) ?? null
      console.log(`[skill-mmanager] Got prompt ${name} from ${info.serverName}`)
      return result
    } catch (error) {
  ...
  private disconnectClientInternal(serverName: string, clientInfo: ClientInfo): void {
    ...
    this.clients.delete(`${serverName}:${clientInfo.client.info.command}`)
  }
  ...
  cleanupIdleClients(): void {
    ...
    for (const [key, clientInfo] of this.clients.entries()) {
      const idleTime = now - clientInfo.lastUsed

      if (idleTime >= IDLE_TIMEOUT) {
        console.log(`[skill-mcp-manager] Disconnecting idle client: ${key}`)
        this.disconnectClientInternal(key, clientInfo)
      }
    }
  }
}
