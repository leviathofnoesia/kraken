...
import type { MCPServerDefinition, MCPTool, Context7Config, DocumentationResult } from './types'
import { RateLimiter, MCPTimeoutError } from './types'

const z = tool.schema

// Context7 API configuration
const CONTEXT7_API_BASE_URL = 'https://api.context7.io/v1'
...
export async function initializeContext7MCP(config: Record<string, unknown> = {}): Promise<void> {
  ...
  if (!currentConfig.apiKey && !process.env.CONTEXT7_API_KEY) {
    console.warn(
      'Context7 MCP: No API key provided. Set CONTEXT7_API_KEY environment variable or provide apiKey in config.',
    )
  }
}
...
async function searchDocumentation(
  ...
  if (!apiKey) {
    throw new Error(
      'CONTEXT7_API_KEY is required for documentation search. Please set the environment variable.',
    )
  }
...
async function getDocumentation(
  ...
  if (!apiKey) {
    throw new Error(
      'CONTEXT7_API_KEY is required for documentation lookup. Please set the environment variable.',
    )
  }
...
const context7SearchTool = tool({
  description:
    'Search official documentation for libraries, SDKs, and APIs. Returns relevant documentation pages with content.',
  args: {
...
const context7GetTool = tool({
  description:
    'Get specific documentation page by library and path. Fetches the exact documentation content.',
  args: {
...
export const context7MCP: MCPServerDefinition = {
  name: 'context7',
  description:
    'Official documentation lookup for libraries, SDKs, and APIs with intelligent caching',
  version: '1.0.0',
